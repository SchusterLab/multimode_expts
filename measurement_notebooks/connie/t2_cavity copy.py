...
# class CavityRamseyProgramFixed(MMRAveragerProgram):
...
#         use_user_defined, user_defined_freq, user_defined_gain, user_defined_sigma, user_defined_length, ramsey_ch = cfg.expt.user_defined_pulse
...
#         run_stor_ramsey, stor_num, run_coupler
...
#         # else:
#         # print(f'phase update channel: {self.cavity_ch}')
#         self.phase_update_page = [self.ch_page(self.cavity_ch[qTest])]
#         self.r_phase = self.sreg(self.cavity_ch[qTest], "phase")
...
#             self.user_freq = self.freq2reg(user_defined_freq, gen_ch=self.cavity_ch[qTest])
#             self.user_gain = user_defined_gain
#             self.user_sigma = self.us2cycles(user_defined_sigma, gen_ch=self.cavity_ch[qTest])
#             self.user_length  = self.us2cycles(user_defined_length, gen_ch=self.cavity_ch[qTest])
...
#         phase_step = self.deg2reg(phase_step_deg -85, gen_ch=self.cavity_ch[qTest]) # phase step [deg] = 360 * f_Ramsey [MHz] * tau_step [us]
...
#                 phase_step = self.deg2reg(85, gen_ch=self.cavity_ch[qTest]) # phase step [deg] = 360 * f_Ramsey [MHz] * tau_step [us]
#                 remaining_phase -= 85
#             else:
#                 phase_step = self.deg2reg(remaining_phase, gen_ch=self.cavity_ch[qTest])
#                 remaining_phase = 0
...
#     def body():
#         # set the mode/channel to do phase updates with a str
        
#         if kerr:
#             # the mode to displace (when doing kerr) is always man even if phase_update_ch is stor
#             if cross_kerr:
#                 # prepare cross kerr mode:
#                 # prepare qubit in 1, swap to man (if cross_kerr_mode is man or stor), swap to stor (if cross_kerr_mode is stor)
        
#         else:
#             # have the option to do ramsey either with displacement or fock state preparation
           
                
    


# class CavityRamseyExperimentFixed(Experiment):
#     """
#     Ramsey experiment
#     Experimental Config:
#     expt = dict(
...
#         ramsey = CavityRamseyProgramFixed(soccfg=self.soccfg, cfg=self.cfg)
...
# class CavityRamseyGainSweepExperimentFixed(Experiment):
...
#         if fit: 
#             cavity_ramsey_analysis = CavityRamseyGainSweepFittingFixed(
#                 data, config=self.cfg, 
#             )
