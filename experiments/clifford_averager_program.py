...
class CliffordAveragerProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.cfg.update(self.cfg.expt)
        if 'qubit' in self.cfg.expt: self.qubits = self.cfg.expt.qubit
        else: self.qubits = [0]
        ...
        for q in self.qubits:
            ...
            # print('swept Q is ',q)
            # print('selfqubits is ',self.qubits)
            self.declare_gen(ch=self.res_chs[q], nqz=self.cfg.hw.soc.dacs.readout.nyquist[q], mixer_freq=mixer_freq)
            self.handle_const_pulse(name=f'measure{q}', ch=self.res_chs[q], length=self.readout_lengths_dac[q], freq_MHz=self.cfg.device.readout.frequency[q], phase_deg=self.cfg.device.readout.phase[q], gain=self.cfg.device.readout.gain[q], play=False, set_reg=True)
