...
DEPRECATED. Moved from `experiments/qsim/notebook_helpers/mbr_disorder_campaign.py`
on 2026-09-24 (MBR redesign step 7c), without changes: the old-class 7-1
functions. The live 7-1 planning, on `MBRSpectrumExperiment` and
`MBRDisorderEnsembleExperiment`, is the new
`notebook_helpers/mbr_disorder_campaign.py`; see `docs/qsim/mbr_step7_plan.md`.
Not maintained; may break when live code changes. If it breaks, add a note
here and do not fix it. Known problems when moved: `best_self_kerr_kHz` is
read through `globals()`, so it is never used; `diag_analysis_error` is
shadowed by the `except` name.
