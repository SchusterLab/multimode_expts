...
class WignerTomography1ModeProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.hpisigma_ge_fast = self.us2cycles(cfg.device.qubit.pulses.hpi_ge_fast.sigma[qTest], gen_ch=self.qubit_chs[qTest]) # default hpi_ge value
        ...
        self.gain_hge_init_fast = self.cfg.device.qubit.pulses.hpi_ge_fast.gain[qTest]
        ...
        self.add_gauss(ch=self.qubit_chs[qTest], name="hpi_qubit_fast", sigma=self.hpisigma_ge_fast, length=self.hpisigma_ge_fast*4)
    ...
    def body(self):
        ...
        self.setup_and_pulse(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ge_init_reg, phase=self.deg2reg(0), gain=self.gain_hge_init_fast, waveform="hpi_qubit_fast")       
...
class WignerTomography1ModeExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        for alpha in tqdm(alpha_list, disable=not progress):
            ...
            scale = 0.05   # parity gain calibration Gaussian pulse length here (in unit of us)
