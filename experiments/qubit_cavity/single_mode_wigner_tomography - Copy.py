...
class WignerTomography1ModeProgram(AveragerProgram):
    ...
    def body(self):
        ...
        # # pre pulse
        # if self.cfg.expt.pre_pulse[0]:
        #     self.setup_and_pulse(ch=self.man_chs[0], style="arb", freq=self.f_cavity, 
        #                     phase=self.deg2reg(self.deg2reg(self.cfg.expt.pre_pulse[3])), 
        #                     gain=self.cfg.expt.pre_pulse[2], waveform="pre_pulse")
...
class WignerTomography1ModeExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        I_list = np.array(self.cfg.expt["I_list"])
        Q_list = np.array(self.cfg.expt["Q_list"])
        gain2alpha = self.cfg.expt.gain2alpha

        data={"Ipts":[],"Qpts":[], "avgi":[], "avgq":[], "amps":[], "phases":[], "i0":[], "q0":[]}

        for II_value in tqdm(I_list, disable=not progress):
            for QQ_value in tqdm(Q_list, disable=not progress):
                # scale =  1.764162781524843     # np.sqrt(np.pi)*erf(np.sqrt(2)) for 1us const pulse
                scale = 0.05   # pu gain calibration Gaussian pulse length here (in unit of us)
                # self.cfg.expt.amp_placeholder =  int(np.abs(II_value+1j*QQ_value)/gain2alpha*scale*self.cfg.expt.displace_length) # scaled, reference is 1us const pulse
                self.cfg.expt.amp_placeholder =  int(np.abs(II_value+1j*QQ_value)/gain2alpha*scale/self.cfg.expt.displace_length) # scaled, reference is 0.05us G pulse
                self.cfg.expt.phase_placeholder = np.angle(II_value+1j*QQ_value)/np.pi*180
                ...
                data["Ipts"].append(II_value)
                data["Qpts"].append(QQ_value)
